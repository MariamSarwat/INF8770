...
def main():
    ...
    for imgPath in IMAGE_PATH:
        ...
        if imgPath == 'data/jpeg':
            imgFoundthreshold = 0.2
        else :
            imgFoundthreshold = 0
        for imageToFind in currentImgDir.iterdir():
            ...
            for videoHists in IndexVidHistDir.iterdir():
                ...
                for frameInfo in json_object:
                    ...
                    eucDistanceFrameImage = hist.get_chebyshev_distance(imgHist, frameHist)
                    ...
                    if(eucDistanceFrameImage < minEucDistance):
                        ...
                        if(eucDistanceFrameImage == 0):
                            found = True
                            break
                if found:
                    break
                    
            if (imgPath == 'data/png' and not found) or (imgPath == 'data/jpeg' and (minEucDistance > imgFoundthreshold)):
                timeInVideo = ''
                videoName = 'out'
                print('Not found: ' + str(imageToFind.stem))
